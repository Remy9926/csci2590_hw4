...
def do_train(args, model, train_dataloader, save_dir="./out"):
    ...
    for epoch in range(num_epochs):
        for data in tqdm(train_dataloader, desc=f"Training model on Epoch {epoch}"):
            labels = data["labels"].to(device)
            input_ids = data["input_ids"].to(device)
            token_type_ids = data["token_type_ids"].to(device)
            attention_mask = data["attention_mask"].to(device)

            outputs = model(input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask)["logits"]
        ...
        print(f"Finished with Epoch {epoch}")
...
def create_augmented_dataloader(args, dataset):
    ...
    indices = [i for i in range(total_items)]
    random.shuffle(indices)

    training_data = dataset["train"].select(indices[0 : 5000])
    training_data_transformed = training_data.map(custom_transform, load_from_cache_file=False)
    unchanged = dataset["train"].select(indices[5000: ])
    augmented_training_data = datasets.concatenate_datasets((training_data_transformed, unchanged))
    transformed_tokenized_dataset = augmented_training_data.map(tokenize_function, batched=True, load_from_cache_file=False)
